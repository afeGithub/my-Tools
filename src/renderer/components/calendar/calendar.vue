<template>
    <div >
        ...
        <div id="week">
            <div class="weekItem"><span style="color: red">日</span></div>
...
<script>
    ...
    export default {
        ...
        mounted() {//页面加载完成后执行这里面的代码
            let firstDay = new Date(Date.UTC(new Date().getFullYear(),new Date().getMonth(), 1)).getDay();
            let endDay =0;
            ...
            if(currentMonth ===2){
                ...
                endDay = big.indexOf(currentMonth)!==-1?31:30
            }
            let day =1;
            for(let i=0;i<35;i++){
                let  solar2lunarObj = this.solar2lunar(new Date().getFullYear(),new Date().getMonth()+1,day);
                let lunarCalendar = this.toChinaDay(solar2lunarObj.lDay);
                let isHoliday = (solar2lunarObj.nWeek === 6 || solar2lunarObj.nWeek === 7);
                if(i<7){
                    if(i<firstDay){
                        this.row1.push({Calendar:'',lunarCalendar:'',isHoliday:false,isSolarTerms:false})
                    }else{
                        this.row1.push({Calendar:day<10?"0"+day:day,lunarCalendar:lunarCalendar,isHoliday:isHoliday,isSign:false})
                        day++;
                    }
                }else if(i>=7 && i<14){
                    this.row2.push({Calendar:i<10?"0"+day:day,lunarCalendar:lunarCalendar,isHoliday:isHoliday,isSign:false})
                    day++;
                }else if(i>=14 && i<21){
                    this.row3.push({Calendar:i<10?"0"+day:day,lunarCalendar:lunarCalendar,isHoliday:isHoliday,isSign:false})
                    day++;
                }else if(i>=21 && i<28){
                    this.row4.push({Calendar:i<10?"0"+day:day,lunarCalendar:lunarCalendar,isHoliday:isHoliday,isSign:false})
                    day++;
                }else if(i>=28 && i<35){
                    if(i<=endDay){
                        this.row5.push({Calendar:i<10?"0"+day:day,lunarCalendar:lunarCalendar,isHoliday:isHoliday,isSign:false})
                        day++;
                    }else{
                        this.row5.push({Calendar:'',lunarCalendar:'',isHoliday:false,isSolarTerms:false})
                    }
                }
            }
